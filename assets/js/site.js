/* موقع طيّبات للأغذية — يقرأ env.js ويملأ الروابط والبيانات */
(function () {
  "use strict";
  ...
  /* القيمة التي على صورة [نص] عنصر نائب لم يُملأ بعد: نترك النائب المكتوب
     في الصفحة كما هو، فيظهر بلغة الصفحة بدل أن تتسرّب العربية إلى الإنجليزية. */
  function isPlaceholder(v) { return /^\[[^\]]*\]$/.test(v); }
  ...
  function socialUrl(key) {
    if (key === "whatsapp") return mainWa ? waHref(mainWa) : "";
    return val(SOCIAL_VARS[key]);
  }
  ...
  document.querySelectorAll("[data-contact-list]").forEach(function (list) {
    ...
    rows.push(["whatsapp", pick("واتساب", "WhatsApp"),
      pick("راسلنا مباشرة على واتساب", "Message us directly on WhatsApp"),
      mainWa ? waHref(mainWa) : ""]);
